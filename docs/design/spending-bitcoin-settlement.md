...
The page creates an isolated SDK wallet with in-memory repositories for one payment. The phone key exists only during the passkey ceremony. One signer session constructs the registration intent, advertises the same tree-signing key, validates the batch tree and builds the forfeit. The settlement stream is subscribed before the Guardian registers the intent, then `Batch.join()` participates directly in that stream. The existing named Guardian program authorizes the exact input, Bitcoin outputs, fee, protected Spending change and registration expiry.

A narrow provider adapter routes registration and final submission through the Guardian. The cancellation proof is created and retained before registration. The batch handler captures the evidence the Guardian independently validates. Generic SDK settlement and intent persistence are omitted for this signing view: their automatic repository and cancellation lifecycle cannot represent an ambiguous Guardian operation or its authorization boundary. The durable Guardian ledger and existing local Bitcoin journal remain authoritative. No phone signing key or generic background retry is introduced.
...
Reference implementations: Vaulted's last working explicit protected-payment flow at `47f45517`, Arkade Wallet `src/lib/asp.ts`, and the pinned SDK's `Batch.join` and settlement handlers. Arkade Wallet and generic `Wallet.settle()` do not implement Vaulted's Guardian authorization and finalization boundary. The SDK archive and enrollment contracts remain unchanged.
