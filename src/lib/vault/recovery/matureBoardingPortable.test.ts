...
describe('independent mature boarding recovery', () => {
  it.each(['mainnet', 'mutinynet'] as const)(
    ...
    async (network) => {
      ...
      const { f, live, file, txid } = await signedPortableMatureBoarding(network)
      ...
      const exportPhone = scalarSecret(3)
      const exportKey = await recoveryBackupKey(exportPhone, file.header)
      exportPhone.fill(0)
      const portable = JSON.parse(JSON.stringify(await createPortableRecoveryPackage(file, exportKey)))
      ...
      let opened
      try {
        const recoveredKey = await recoveryBackupKey(phone, pkg.backup.header)
        opened = validateVaultRecoveryFile(await decryptRecoveryBackup(pkg.backup, recoveredKey))
      } finally {
        phone.fill(0)
      }
      ...
      await expect(executeMatureBoardingRecoveryFile(alteredBytes, bitcoin)).rejects.toThrow()
      ...
      await expect(executeMatureBoardingRecoveryFile(altered, bitcoin)).rejects.toThrow()
      expect(bitcoin.broadcastTransaction).toHaveBeenCalledTimes(1)
      ...
      await expect(executeMatureBoardingRecoveryFile(foreign, bitcoin)).rejects.toThrow()
      expect(bitcoin.broadcastTransaction).toHaveBeenCalledTimes(1)
    },
  )
})
