...
import { LightScript, lightDescriptorDigest, validateLightDescriptor, type LightDescriptor } from './contract'

export interface GuardianDelegateInfo extends DelegateInfo {
  version: 1
  enabled: true
  maxInputs: 1
  maxScheduleSeconds: number
}
export interface GuardianScheduleRequest {
  vaultId: string
  operationId: string
...
export interface GuardianDelegationPlan {
  request: GuardianScheduleRequest
  txid: string
  vout: number
  valueSats: number
  receiverSats: number
...
export function delegationDigest(purpose: 'schedule' | 'status' | 'cancel' | 'list', value: unknown) {
  return sha256(new TextEncoder().encode(`vaulted-light/delegate-${purpose}/v1:${JSON.stringify(value)}`))
}
export function lightDelegationAddress(d: LightDescriptor) {
  return new ArkAddress(
    hex.decode(d.operatorPub),
    new LightScript(d).tweakedPublicKey,
    networkPins(d.network).arkHrp,
  ).encode()
}
export function validateGuardianDelegateInfo(value: unknown, descriptor: LightDescriptor): GuardianDelegateInfo {
  const d = validateLightDescriptor(descriptor),
    v = value as GuardianDelegateInfo
  if (
    !v ||
    v.enabled !== true ||
    v.version !== 1 ||
    ...
    v.fee !== '0' ||
    v.delegateAddress !== lightDelegationAddress(d) ||
...
export function delegationScheduleBody(r: GuardianScheduleRequest) {
  return {
    vaultId: r.vaultId,
...
export function validateGuardianSchedule(r: GuardianScheduleRequest, d: LightDescriptor) {
  ...
  if (
    ...
    !schnorr.verify(
      ...
      delegationDigest('schedule', delegationScheduleBody(r)),
  ...
  const script = new LightScript(d)
  if (
  ...
  const leafHash = tapLeafHash(hex.decode(script.spendScript))
...
export async function prepareGuardianDelegation(
  ...
  try {
    ...
    const request = { ...body, ownerSignature: hex.encode(schnorr.sign(delegationDigest('schedule', body), owner)) }
    const facts = validateGuardianSchedule(request, d)
