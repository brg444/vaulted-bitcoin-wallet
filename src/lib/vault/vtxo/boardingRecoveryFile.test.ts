...
describe('standalone named boarding recovery', () => {
  it.each(['mainnet', 'mutinynet'] as const)(
    'uses the SDK phone recovery leaf and resumes the signed transaction on %s',
    async (network) => {
      ...
      try {
        await expect(
          prepareBoardingRecoveryFile(archive, { txid: parent.id, vout: 0 }, scalarSecret(4), chain),
        ).rejects.toThrow('identity')
        const file = await prepareBoardingRecoveryFile(archive, { txid: parent.id, vout: 0 }, scalarSecret(3), chain)
