...
import { validateRenewalStatusBinding, requireSpendingRenewalRecovery } from './renewalStatus'
...
export function requireSpendingRenewalAncestry(
  ...
  return requireDelegationInputAncestryForBinding(binding(status), coin, info, indexer, () =>
...
export function importSpendingRenewalReplacement(
  ...
  return importDelegationReplacementForBinding(binding(status), result, info, coin, () =>
    vaultExitRepository(status.vaultId, status.network),
  )
}

async function requireDelegationInputAncestryForBinding(
  binding: { network: VaultNetwork; descriptorHash: string; scriptPubKey: string },
  coin: VirtualCoin,
  info: ArkInfo,
  indexer: IndexerProvider,
  createRepository: () => ReturnType<typeof vaultExitRepository>,
) {
...
async function importDelegationReplacementForBinding(
  binding: {
  ...
  response: SpendingRenewalStatus,
  operatorInfo: ArkInfo,
  coin: VirtualCoin,
  repository: () => ReturnType<typeof vaultExitRepository>,
) {
  const d = structuredClone(binding),
    status = validateRenewalStatusBinding(response, d)
  const recovery = requireSpendingRenewalRecovery(status),
