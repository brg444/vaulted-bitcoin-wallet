...
describe('vault UI lock', () => {
  ...
  it('keeps Standard and Advanced protection chrome and the kit path', () => {
    expect(existsSync(resolve(root, 'src/screens/Vault/onboard/Ledger.tsx'))).toBe(true)
    expect(existsSync(resolve(root, 'src/screens/Vault/Recover.tsx'))).toBe(true)
    const recovery = read('src/screens/Vault/onboard/Ledger.tsx')
    expect(recovery).toMatch(/Public recovery account/)
    expect(recovery).not.toMatch(/Prove you hold/)
    expect(recovery).not.toMatch(/recovery-secret/)
    expect(recovery).toMatch(/separate seed backup/)
    ...
    const app = read('src/VaultApp.tsx')
    expect(app).toContain('LedgerRecoveryKey')
    expect(app).toContain('VaultLedgerPayment')
    ...
    expect(enroll).toMatch(/Ledger recovery account does not match the selected protection/)
    expect(enroll).toContain('buildLedgerRecoveryDescriptor')
