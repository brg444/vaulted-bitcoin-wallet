...
describe('Transaction reference', () => {
  ...
  it('keeps the full ID selectable when clipboard access is unavailable', async () => {
    ...
    fireEvent.click(screen.getByText('View transaction'))
    fireEvent.click(screen.getByRole('button', { name: 'Copy transaction ID' }))
    await waitFor(() => expect(screen.getByRole('status')).toHaveTextContent('Select the transaction ID'))
  ...
  it('does not carry copy failure into a different transaction', async () => {
    vi.stubGlobal('navigator', {})
    const { rerender } = render(<TransactionReference txid={'ab'.repeat(32)} explorer={null} />)
    fireEvent.click(screen.getByText('View transaction'))
    fireEvent.click(screen.getByRole('button', { name: 'Copy transaction ID' }))
    await waitFor(() => expect(screen.getByRole('status')).toHaveTextContent('Select the transaction ID'))
    rerender(<TransactionReference txid={'cd'.repeat(32)} explorer={null} />)
    expect(screen.getByRole('status')).toBeEmptyDOMElement()
    expect(screen.getByRole('button', { name: 'Copy transaction ID' })).toHaveTextContent('Copy ID')
  })
})
