...
for (const width of [320, 390, 1280]) {
  test(`Lightning address remains readable at ${width}px`, async ({ page }, testInfo) => {
    ...
    await page.getByLabel('Address name', { exact: true }).fill('Alex')
    await expect(page.getByText('alex@ln.getvaulted.xyz', { exact: true })).toBeVisible()
    ...
    await page.getByLabel('Address name', { exact: true }).fill('alex-bergeron')
    await expect(page.getByRole('button', { name: 'Save address' })).toBeEnabled()
    await expect(page.getByText('Previously shared addresses will keep working for this wallet.')).toBeVisible()
